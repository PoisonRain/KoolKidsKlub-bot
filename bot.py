...
def handle_defense_elf(game):
    ...
    Defense_portal_location = game.get_my_castle().location.towards(game.get_enemy_castle().location, 1000) #the location
        ...
        if game.get_enemy_portals() is not None:
            for portal in game.get_enemy_portals():
                if defense_elf.in_attack_range(portal):
                    ...
                    elf_acted = True
                    break
                elif game.get_my_castle().in_range(portal, 600):
                    defense_elf.attack(portal)
                    elf_acted = True
                    break
...
def defense_portals(game):
    ...
    if my_portals != None:
        ...
        for portal in my_portals:
            if my_castel.distance(portal) < closest_portal:
                closest_portal = portal
        defense_ports = [closest_portal]
    else:
        defense_ports = []
    for portal in my_portals:
        if my_castel.distance(portal) < 1000:
            defense_ports.append(portal)

        for portal in defense_ports:
            for giant in enemy_giants:
                if portal.in_range(giant, game.lava_giant_attack_range + 4*game.lava_giant_max_speed) and portal.can_summon_ice_troll():
                    print('summon ice troll in defense portal against lava')
                    portal.summon_ice_troll()
            for elf in game.get_enemy_living_elves():
                if portal.in_range(elf, game.elf_attack_range + 4*game.elf_max_speed) and portal.can_summon_ice_troll():
                    print('summon ice troll in defense portal again elf')
                    if game.get_my_mana() > 100:
                        portal.summon_ice_troll()
...
def summon_ice(game, my_portals, my_castel):
    if my_portals[0] != None:
        ...
        if SPAWN_ICE and closest_portal.can_summon_ice_troll() and my_castel.distance(closest_portal) < PORTAL_DEFENSE_DISTANCE:
            print"2"
            closest_portal.summon_ice_troll()
