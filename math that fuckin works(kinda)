...
def lineFromPoints(point1,point2):
    ...
    if (a < 0):
        print("The line passing through the points:"+str(b)+"x "+str(a)+"y = "+str(c)+"\n")
    else:
        print("The line passing through the points: "+str(b)+ "x + "+ str(a)+ "y = "+str(c)+ "\n")
...
def points_on_circle(center,r,n=500): #default amount of points is 100
    return [(math.cos(2*pi/n*x)*r+center[0],math.sin(2*pi/n*x)*r+center[1]) for x in range(0,n+1)]
...
def do_turn(game):
    ...
    global prev_a
    if game.turn <= 1:
        elf_start = (test_elf.location.row, test_elf.location.col)
       
    else:
        manuver_to = get_side_for_maneuver(elf_start, enemy_castle_loc_test, (2200, 3000))
        print enemy_castle_loc_test
        print elf_start
        print manuver_to
        center, radius = get_circle(elf_start, manuver_to, enemy_castle_loc_test)
        if points == []:
            points = points_on_circle(center, radius)
        else:
            print radius, center
            next_tuple = points[i]
            i+=1
            next_point = Location(int(round(next_tuple[0])), int(round(next_tuple[1])))
            test_elf.move_to(next_point)
